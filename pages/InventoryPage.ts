...
export class InventoryPage extends BasePage {
  ...
  async addItemToCart(productName: string) {
    ...
    const item = this.page.locator(this.inventoryItem, {
      hasText: productName,
    });
  ...
  async validateProductDescriptions() {
    const descs = await this.page
      .locator(this.inventoryItemDesc)
      .allInnerTexts();
  ...
  async validateProductButtons(buttonText: string) {
    ...
    texts.forEach((t) =>
      expect(t.toLowerCase()).toBe(buttonText.toLowerCase()),
    );
  }
  ...
  async validateFooterCopy() {
    ...
    await expect(this.page.locator(this.footerCopy)).toContainText(
      "Sauce Labs",
    );
  }
}

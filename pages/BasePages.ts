  ...
  async smartClick(selector: string, contextDescription: string) {
    try {
      ...
      try {
        ...
        let suggestedSelector = match
          ? match[1]
          : analysis.startsWith("#") || analysis.startsWith(".")
            ? analysis.trim()
            : null;

        if (suggestedSelector && suggestedSelector !== "null") {
          console.log(
            `[Self-Healing] ✅ Tentando novo seletor: ${suggestedSelector}`,
          );
          ...
          await this.page.waitForSelector(suggestedSelector, {
            state: "visible",
            timeout: 5000,
          });
